...
EXCLUDE_ENTITIES_TYPE = [u'DATE',u'TIME',u'PERCENT',u'QUANTITY',u'CARDINAL']
...
def _getCandidateFromChunkText(chunk_text,pos_tagging,chunk_text_max_len=3):
    ...
    if len(chunk_text.split())>chunk_text_max_len:
        chunk_text = ""
    return chunk_text
...
def run(title,article_content,article_id,save_predictors = False):
    ...
    if save_predictors:
        ...
        ADDED_ARTICLE_PREDICTORS = candidatesUtils.getCandidatePredictorsAsDF(candidates_predictors, article_id)
        ADDED_ARTICLE_PREDICTORS["approved"] = [False]*len(ADDED_ARTICLE_PREDICTORS)
        for candidate in candidates:
            ADDED_ARTICLE_PREDICTORS.loc[(article_id,candidate),"approved"] = True
        PREDICTORS_DF = PREDICTORS_DF.append(ADDED_ARTICLE_PREDICTORS)
    return candidates
...
if __name__ == '__main__':
    ...
    for i,article in dat.iterrows():
        ...
        print "#################"
        print article.title
        article_content = literal_eval(article.article_content)
        print "extracted keywords :"
        print run(article.title,article_content,article.article_id,True)
        print "tags :"
        print literal_eval(article.tech_crunch_tags)
    print "done, can print predictors_df now"
    PREDICTORS_DF.to_csv(r"C:\Users\Alon Arad\Documents\NI\pred_dat_2.csv")
    pass